...
public class HelloWorldApplication {
	...
	public static void main(String[] args) {
		 ...
		 * 2. 만약 웹 애플리케이션이면, 웹 애플리케이션 타입을 결정(SpringMVC or Rective)
		 * 3. 어노테이션 스캐닝(Auto) + Configuration Class를 통한 빈 생성/등록/와이어링 작업
		 * 4. 만약 웹 애플리케이션이고 타입이 Spring MVC이면 
		 ...
		 * 
		 * try { ac = SpringApplication.run(HelloWorldApplication.class, args); }
		 * catch(Throwable ex) { ex.printStackTrace(); } finally { if(ac != null) {
		 * ac.close(); } }
		 */
